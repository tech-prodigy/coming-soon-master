					...
					function overlay_loading(control_id)
					{
						...
						if(control_id != undefined)
						{
							switch(control_id)
							{
								...
								case "label_settings":
									var message = "<?php _e("Your data has been saved successfully.",coming_soon_master)?>";
									var success = "<?php _e("Success!",coming_soon_master)?>";
								break;
							}
				...
				case "csm_label_settings":
					...
					({
						...
						submitHandler: function(form)
						{
							jQuery.post(ajaxurl,
							{
								data: jQuery("#ux_frm_label_settings").serialize(),
								param: "label_settings_module",
								action: "coming_soon_master",
								_wp_nonce:"<?php echo $csm_label_settings;?>"
							},
							function()
							{
								overlay_loading("label_settings");
								setTimeout(function()
								{
									remove_overlay();
									window.location.href = "admin.php?page=csm_label_settings";
								}, 3000);
							});
						}
					});
