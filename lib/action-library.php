...
else
{
	...
	else
	{
		if(isset($_REQUEST["param"]))
		{
			...
			case "button_settings_module" :
				if(wp_verify_nonce($_REQUEST["_wp_nonce"], "coming_soon_master_button_settings_cf")))
				{
					parse_str($_REQUEST["data"],$form_data);
					$update_button_settings_cf = array();
					$where = array();

					$update_button_settings_cf["button_label_contact_form"] = esc_attr($form_data["ux_txt_button_label_cf"]);
					$update_button_settings_cf["button_color_contact_form"] = esc_attr($form_data["ux_clr_button_color_cf"]);
					$update_button_settings_cf["hover_color_contact_form"] = esc_attr($form_data["ux_clr_hover_color_cf"]);
					$update_button_settings_cf["font_family_button_contact_form"] = esc_attr($form_data["ux_ddl_font_family_button_cf"]);
					$update_button_settings_cf["label_assignment_contact_form"] = esc_attr($form_data["ux_ddl_label_assignment_cf"]);

					foreach($update_button_settings_cf as $keys => $value)
					{
						$update_button_settings_cf = array();
						$where["meta_key"] = $keys;
						$update_button_value["meta_value"] = $value;
						$obj_dbHelper->updateCommand(coming_soon_meta(),$update_button_value,$where);
					}
				}
				die();
			break;
